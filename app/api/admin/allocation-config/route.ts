...
async function checkAdmin(userId: string) {
  return hasPerms(userId, ['allocation-config'])
}
...
export async function GET(request: NextRequest) {
  ...
  if (!(await hasPerms(user.id, ['allocation-config']))) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
