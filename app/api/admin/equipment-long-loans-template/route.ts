...
export async function GET() {
  ...
  const [{ data: equipment }, { data: loans }, { data: profiles }] = await Promise.all([
    ...
    supabaseAdmin.from('profiles').select('id, name').neq('status', 'inactive').order('name'),
  ])

  const activeByEquipment = new Map((loans ?? []).map(l => [l.equipment_id, l]))
  const teacherName = new Map((profiles ?? []).map(p => [p.id, p.name ?? '']))
  ...
  const rows = (equipment ?? []).map(e => {
    ...
    return {
      設備名稱: e.name,
      設備編號: e.asset_number,
      老師姓名: loan ? (teacherName.get(loan.teacher_id) ?? '') : '',
      起始日: loan?.start_date ?? '',
      到期日: loan?.due_date ?? '',
      備註: loan?.notes ?? '',
    }
  ...
  const NAME_LIST = Array.from(new Set((profiles ?? []).map(p => p.name).filter(Boolean))) as string[]
  ...
  const help = XLSX.utils.aoa_to_sheet([
    ...
    ['老師姓名', '從下拉選單選取。Email 由系統自動比對，不需填寫。'],
