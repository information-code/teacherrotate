...
const CATEGORIES = new Set(['formal', 'substitute', 'hourly'])
...
/** 新增清冊教師。body: { plan_id, teacher_id?, name, category } */
export async function POST(request: NextRequest) {
  ...
  const category = String(body?.category ?? 'formal')
  if (!plan_id) return NextResponse.json({ error: '缺少計畫' }, { status: 400 })
  if (!name) return NextResponse.json({ error: '請填寫教師姓名' }, { status: 400 })
  if (!CATEGORIES.has(category)) return NextResponse.json({ error: '身分無效' }, { status: 400 })
...
/** 修改教師（身分、代扣款、備註）。body: { id, category, labor_fee, health_fee, lunch_fee, other_fee, note } */
