...
const RULE_ROWS: { key: SimpleKey | ParamKey; hasN?: boolean; name: string; tag: string; def: string; desc: string }[] = [
  ...
  { key: 'subjectSpread', name: '同科不隔天', tag: '班級', def: '中', desc: '同班同科盡量不排相鄰兩天（同科同日仍為固定硬限制）。依 114-2 人工課表檢核：48 班有違反、一週 ≥4 節的科目結構上無解，故為權重不為硬限制' },
  { key: 'classCohesion', name: '科任課同日成塊', tag: '導師', def: '中', desc: '同班同日（上、下午各自計）科任課與鎖課盡量連成一塊，導師課不被切碎。依人工課表檢核 32 班有違反、且與「空堂最多一段」互斥，故為權重' },
    ...
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="card p-3 space-y-1">
        ...
        <ul className="text-xs text-zinc-500 list-disc pl-5 space-y-0.5">
          ...
          <li className="text-zinc-400">「同科不隔天」「科任課同日成塊」已依 114-2 人工課表檢核降為可調權重（見下）</li>
        </ul>
      </div>
