...
const GROUPS: { title: string; note: string; rows: RuleRow[] }[] = [
  { title: '導師', note: '為導師而設：留白落在哪裡、每天要上幾節、會不會被切碎', rows: [
    ...
    { key: 'homeroomMorningMax', name: '上午導師課上限', def: '高', desc: '課務組原則：沒有鎖課的老師上午最多 3 節導師課（可以連 3）；導師自己的鎖課（種子班國數）在上午就超過 3 的，以鎖課數為準。人工課表 5 節日以 3+2／2+3 為主、4+1 偶有。視藝單雙週導師週多出的兩節不算（單雙週結構逼的，與鎖課同理）。勾「必須級」＝超過就卡成功條件並點名；另外「半天日整個半天都是導師課」（連上四節、一堂科任都沒有）在勾必須級時一律不允許——人工課表四期 566 個半天日只有 1 個' },
