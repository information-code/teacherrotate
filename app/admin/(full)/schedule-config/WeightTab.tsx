...
const GROUPS: { title: string; note: string; rows: RuleRow[] }[] = [
  { title: '導師', note: '為導師而設：留白落在哪裡、每天要上幾節、會不會被切碎', rows: [
    ...
    { key: 'homeroomMorningMax', name: '上午導師課上限', def: '高', desc: '沒有鎖課的班上午最多 3 節導師課（可以連 3）；導師自己的鎖課（種子班國數）在上午就超過 3 的以鎖課數為準。視藝單雙週導師週多出的兩節不算。預設只給權重高、不勾必須級——人工課表四期每期有 6～15 個班日是「上午四節全導師」，不是鐵律。另外「半天日整個半天都是導師課」（連上四節、一堂科任都沒有）不論有沒有勾必須級都一律不允許——人工課表四期 566 個半天日只有 1 個' },
                ...
                <div className="flex items-start gap-3">
                  ...
                  {r.key === 'homeroomDailyMax' && on && (
                    <div className="flex items-center gap-2 text-xs text-zinc-500 flex-shrink-0 self-center flex-wrap justify-end">
                      ...
                      <label className="flex items-center gap-1" title="中高年級導師一天絕不超過此數（必須級）。人工課表四期中高年級 0 筆">絕對上限
                      ...
                      <label className="flex items-center gap-1" title="低年級只有週二是整天，一週 7～8 堂科任分五天、週二只分到 1～2 堂，導師必然多上——114 人工課表 2年9班 週二就是 6 節">低年級整天日絕對上限
                        <input type="number" min={1} max={7} value={w.builtin.homeroomDailyMax.hardFullDayLowN}
                          onChange={e => setBuiltin({ homeroomDailyMax: { ...w.builtin.homeroomDailyMax, hardFullDayLowN: Math.min(7, Math.max(1, Number(e.target.value) || 6)) } })}
                          className="input w-12 text-center py-0.5 text-xs" />節
                      </label>
