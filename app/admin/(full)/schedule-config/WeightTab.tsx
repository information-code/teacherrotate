...
const GROUPS: { title: string; note: string; rows: RuleRow[] }[] = [
  ...
  { title: '科任・行政', note: '為授課老師本人而設：一週課表的鬆緊、空堂與移動', rows: [
    ...
    { key: 'roomHalfDay', name: '專科教室老師集中', def: '高', desc: '每位老師在同一間專科教室的課盡量集中在少數幾天——一天一位老師，不用每天收實驗器材；老師課數排不滿一天時，剩下那天上午一位、下午一位（甲 1-4／乙 5-6 ✓）；走了不再回來（甲 1-2／乙 3-4／甲 5-6 ✗，自然是硬限制）。同一天每多一位老師扣 1 次、回頭扣 1 次、同半天兩位扣 ½ 次。114-1 人工課表自然教室：每位老師 3 天、每間只有 1 天兩位老師', link: { href: '/admin/schedule-config?tab=room', label: '教室與管理教師在「4 教室設定」' } },
